...
function start(m){mode=m;state={idols:IDOLS.map(x=>({...x,score:1500,games:0,skips:0})),history:[],n:0,target:m==='quick'?100:200};show('#sort');next()}
...
function pick(){
  ...
  for(let i=0;i<pool.length;i++)for(let j=i+1;j<pool.length;j++){
    ...
    c.push({a,b,d:Math.abs(a.score-b.score)+Math.random()*120+(a.skips+b.skips)*80});
  }
  c.sort((x,y)=>x.d-y.d);
...
function choose(t){let a=current.a,b=current.b;snapshot=state.idols.map(x=>({id:x.id,score:x.score,games:x.games,skips:x.skips||0}));state.history.push({a:a.id,b:b.id,t,snapshot});if(t==='l')elo(a,b,1,1);else if(t==='lv')elo(a,b,1,LOVE_K);else if(t==='r')elo(a,b,0,1);else if(t==='rv')elo(a,b,0,LOVE_K);else if(t==='b'){a.score+=BOTH_BASE;b.score+=BOTH_BASE;a.games++;b.games++}else if(t==='bl'){a.score+=BOTH_LOVE;b.score+=BOTH_LOVE;a.games++;b.games++}else if(t==='s'){a.games++;b.games++;a.skips=(a.skips||0)+1;b.skips=(b.skips||0)+1}state.n++;next()}
function undo(){let h=state.history.pop();if(!h)return;state.idols.forEach(x=>{let s=h.snapshot.find(y=>y.id===x.id);x.score=s.score;x.games=s.games;x.skips=s.skips});state.n--;current={a:state.idols.find(x=>x.id===h.a),b:state.idols.find(x=>x.id===h.b)};render()}
